...
class GitHubScanner:
    ...
    def _fetch_projects_from_github(self, query: str) -> List[Dict[str, Any]]:
        ...
        raw_project_data = [
            {"id": 1, "name": f"project_A_from_{query.replace(':', '_')}", "stars": 100, "url": "http://example.com/a"},
            {"id": 2, "name": f"project_B_from_{query.replace(':', '_')}", "stars": 200, "url": "http://example.com/b"}
        ]
    ...
    def _process_and_catalogue_projects(self, raw_projects: List[Dict[str, Any]]) -> pd.DataFrame:
        """
        ...
        """
        logger.info(f"Processing {len(raw_projects)} raw projects into a DataFrame. (Run ID: {self.run_id})")
        catalogued_projects = []
    ...
    def scan_and_save_catalog(self, search_queries: List[str], output_filename: str = "catalogued_projects.json"):
        """
        ...
        logger.info(f"Successfully saved catalog of {len(final_catalog_df)} projects to {output_path}")
        return output_path

    # --- Methods for Government Repositories Scan ---

    def _fetch_gov_github_accounts_yaml(self, url: str) -> Optional[Dict[str, List[str]]]:
    ...
    def _fetch_repository_details_for_account(self, username: str) -> Optional[List[Dict[str, Any]]]:
        ...
        full_repo_details = []
        logger.info(f"Found {len(repos_data)} repositories for {username}. Fetching details...")

        for repo in repos_data:
            if not isinstance(repo, dict): continue # Basic type check
            repo_details = {
            ...
            readme_url = f"https://api.github.com/repos/{username}/{repo_details['name']}/readme"
                ...
                if readme_api_response.status_code == 200:
                    ...
                    if download_url:
                        ...
                        if readme_content_response.status_code == 200:
                            ...
                            logger.debug(f"Failed to download README for {username}/{repo_details['name']}, status: {readme_content_response.status_code}")
                    else:
                        logger.debug(f"No download_url for README of {username}/{repo_details['name']}")
                elif readme_api_response.status_code != 404: # Log if not a simple "Not Found"
                     logger.warning(f"Failed to fetch README metadata for {username}/{repo_details['name']}, status: {readme_api_response.status_code}")
            except requests.exceptions.RequestException as e:
                logger.warning(f"RequestException for README {username}/{repo_details['name']}: {e}")
            except json.JSONDecodeError:
                logger.warning(f"JSONDecodeError for README metadata {username}/{repo_details['name']}")
            
            full_repo_details.append(repo_details)
            # time.sleep(0.1) # Optional: be polite to the API if fetching many READMEs

        logger.info(f"Fetched details for {len(full_repo_details)} repositories for account: {username}")
        return full_repo_details

    def _save_gov_repos_to_markdown(self, gov_repos_data: List[Dict[str, Any]], output_filename: str) -> str:
        ...
        with open(output_path, 'w', encoding='utf-8') as f:
            ...
            f.write('| Account | Repository Name | Description | Stars | Forks | Language | URL | README Snippet |\n')
            f.write('|---------|-----------------|-------------|-------|-------|----------|-----|----------------|\n')
            for repo in gov_repos_data:
                readme_snippet = repo.get('readme_snippet', '')
                # Sanitize for Markdown table: escape pipes, remove newlines
                readme_display = readme_snippet.replace('|', '\|').replace('\n', ' ').replace('\r', '')[:50] + "..."
                description_display = (repo.get('description', 'N/A') or "No description").replace('|', '\|').replace('\n', ' ').replace('\r', '')

                f.write(f"| {repo.get('account', 'N/A')} | {repo.get('name', 'N/A')} | {description_display} | {repo.get('stars', 0)} | {repo.get('forks', 0)} | {repo.get('language', 'N/A')} | [{repo.get('url')}]({repo.get('url')}) | {readme_display} |\n")
        logger.info(f"Saved government repositories catalog to {output_path}")
        return str(output_path)

    def scan_and_save_gov_repos_catalog(self, gov_accounts_url: Optional[str] = None, output_filename: str = "gov_repositories_catalog.md") -> Optional[str]:
        """
        ...
        if not self.api_token or "your_default_token" in self.api_token:
            logger.error("A valid GitHub API token is required to fetch government repository details. Please set GITHUB_API_TOKEN in your environment or config.")
            return None

        gov_yaml_data = self._fetch_gov_github_accounts_yaml(url_to_fetch)
        if not gov_yaml_data: return None
        ...
        if not gov_usernames:
            ...
            if filter_to_apply: log_message += " (After applying filter based on TARGET_REPOS)"
            logger.info(log_message); return None
        ...
        for i, username in enumerate(gov_usernames):
            ...
            repo_details_for_account = self._fetch_repository_details_for_account(username)
        ...
        if not all_repos_details: logger.info("No repository details were fetched from any government account."); return None
